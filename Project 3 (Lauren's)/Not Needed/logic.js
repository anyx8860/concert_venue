var myMap = L.map("map", {
  center: [36.77, -119.41], //starting position
  zoom: 4,
});
var layer = {}
...
d3.csv("ArtistList.csv").then(function (artist) {
  ...
  d3.json(url).then(function (data) {
    ...
    for (var i = 0; i < events.length; i++) {
      ...
      var marck = L.marker([events[i].location.lat, events[i].location.lng]).bindPopup(`events name: ${events[i].displayName}, Location: ${events[i].location.city}, url for tickets: ` + ticket).addTo(myMap)
